...
public class ESContentSearch extends ESContentletAPIImpl {
	...
	public SearchResponse esSearchRaw(String esQuery, boolean live, User user, boolean respectFrontendRoles) throws DotSecurityException,
		...
		ESSearchResults<Contentlet> contents = new ESSearchResults<Contentlet>();
		String indexToHit;
		IndiciesInfo info;
		try {
	...
	public ESSearchResults<Contentlet> esSearch(String esQuery, boolean live, User user, boolean respectFrontendRoles)
		...
		contents.setQueryTook(resp.getTookInMillis());
		QueryType qt = QueryType.search;
		esQuery = esQuery.toLowerCase();
		if (esQuery.contains("\"suggest\"")) {
			qt = QueryType.suggest;
		...
		if (qt == QueryType.Facets) {
			contents.setFacets(resp.getFacets());
		} else if (qt == QueryType.suggest) {
			contents.setSuggestions(resp.getSuggest());
		} else {

			contents.setScrollId(resp.getScrollId());
			contents.setTotalResults(resp.getHits().totalHits());
		}
		int x = resp.getHits().getHits().length;
		contents.setCount(x);
		contents.setHits(resp.getHits());

		contents.setResponse(resp);
