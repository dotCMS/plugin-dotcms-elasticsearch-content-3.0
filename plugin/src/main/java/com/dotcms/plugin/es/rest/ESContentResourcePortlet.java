package com.dotcms.plugin.es.rest;
...
import com.dotcms.plugin.es.elasticsearch.ESContentSearch;
import com.dotcms.plugin.es.elasticsearch.ESSearchResults;
...
import com.dotcms.repackage.org.codehaus.jackson.JsonNode;
import com.dotcms.repackage.org.codehaus.jackson.map.ObjectMapper;
import com.dotcms.repackage.org.codehaus.jackson.map.SerializationConfig.Feature;
...
public class ESContentResourcePortlet extends BaseRestPortlet {
	...
	ESContentSearch esapi = new ESContentSearch();
	...
	public Response search(@Context HttpServletRequest request) {
		...
		try {
			...
			ESSearchResults<Contentlet> esresult = esapi.esSearch(esQuery, live, user, live);
			
			JSONObject json = new JSONObject();
			JSONArray jsonCons = new JSONArray();

			for(Contentlet c : esresult){
				try {
