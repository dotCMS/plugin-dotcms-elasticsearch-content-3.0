...
<%@page import="com.dotcms.plugin.es.elasticsearch.ESSearchResults"%>
<%@page import="com.dotcms.plugin.es.elasticsearch.ESContentSearch"%>
...
ESSearchResults<Contentlet> cons = new ESSearchResults<Contentlet>();
int counter = 1;
...
if(query == null){
	...
	try{
		...
		if (UtilMethods.isSet(query)) {
			ESContentSearch es = new ESContentSearch();

				cons = es.esSearch(query, true,userForPull,true);
			

		}
		else {

			cons = new ESSearchResults<Contentlet>();
		}
...
function refreshPane(){
    	...
    	var data = {"query":dijit.byId("query").getValue(),"userid":dijit.byId("userid").getValue(),"raw": dijit.byId("raw").getValue()};
...
<div class="portlet-wrapper">
			...
			<table class="listingTable" style="width:70%">	
				...
				<tr>
					<th><strong><%= LanguageUtil.get(pageContext, "Raw") %> :</strong></th>
					<td nowrap="nowrap">
					
						<input dojoType="dijit.form.CheckBox" name="raw"  id="raw" type="checkbox" value="true" <%=raw ? "checked=true" : ""%>
			...
			<%if(UtilMethods.isSet(cons.getQuery())){ %>
